...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try 
			{
				...
				PreparedStatement login = (PreparedStatement) connection.prepareStatement("SELECT * FROM bpls_t_employee_profile EP JOIN bpls_t_user U ON EP.EP_ID = U.UEP_ID JOIN bpls_r_division DV ON EP.DIV_CODE = DV.DIV_CODE WHERE U.U_USERNAME = ? AND U.U_PASSWORD = ? AND U.U_STATUS = 'Active'");
				...
				if(resultSet.next())
				{
					...
					{
						RequestDispatcher dispatcher = request.getRequestDispatcher("BPLSRIndex.jsp");
						dispatcher.forward(request, response);
					}
				}
				else
				{
					RequestDispatcher requestDispatcher = request.getRequestDispatcher("Login_page.jsp");
